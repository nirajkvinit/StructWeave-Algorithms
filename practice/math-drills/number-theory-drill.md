...
## Section 0: GCD and LCM Foundations

### Concepts

**Greatest Common Divisor (GCD)**: The largest positive integer that divides both a and b. Also called Highest Common Factor (HCF).

**Euclidean Algorithm**: `gcd(a, b) = gcd(b, a mod b)`, with base case `gcd(a, 0) = a`. Runs in O(log(min(a, b))) time.

**Least Common Multiple (LCM)**: The smallest positive integer divisible by both a and b. Related to GCD by: `lcm(a, b) = a × b / gcd(a, b)`.

**Extended Euclidean Algorithm**: Finds integers x, y such that `ax + by = gcd(a, b)` (Bézout's identity). This is the foundation for modular inverse.

### Problems

#### 0.1 GCD Queries on Array

Given an array of n integers and q queries, each query asks for the GCD of elements in range [l, r]. Support both queries and point updates.

**Example**: Array `[12, 18, 24, 6]`, query `[0, 2]` → `gcd(12, 18, 24) = 6`

**Approach Hints:**
1. Naive: O(q × n) - compute GCD for each query
2. Sparse Table: O(n log n) preprocessing, O(1) per query (no updates)
3. Segment Tree: O(n) build, O(log n) per query, supports updates

**Key Insight**: GCD is associative and idempotent (gcd(a, a) = a), making it perfect for sparse table optimization.

---

#### 0.2 Frog Jumps on Circular Pond

A frog sits on position 0 of a circular pond with n lily pads (numbered 0 to n-1). It can jump exactly k positions clockwise. Determine how many unique lily pads the frog can visit before returning to start.

**Example**: n = 12, k = 8
- Visits: 0 → 8 → 4 → 0 (cycle completes)
- Answer: 3 unique pads

**Formula**: `n / gcd(n, k)`

**Key Insight**: The frog visits all positions in the cyclic subgroup generated by k modulo n. The orbit size equals the index of this subgroup.

**Follow-up**: What if the frog can jump either k positions clockwise OR m positions counter-clockwise?

---

#### 0.3 LCM of Large Sequence

Given a sequence of n positive integers, compute their LCM modulo 10^9 + 7.

**Challenge**: LCM grows exponentially. For `[2, 3, 4, 5, 6]`, LCM = 60. But for first 20 positive integers, LCM = 232,792,560.

**Why naive approach fails**: You cannot compute the actual LCM then take mod (overflow + modular division issues).

**Approach Hints:**
1. Track prime factorization: `LCM = ∏(p^max_power)` across all primes
2. For each number, factorize and update max power for each prime
3. Compute final answer using modular exponentiation

**Key Insight**: Unlike GCD, LCM doesn't distribute nicely over modular arithmetic. You must work with prime factorizations.

---

#### 0.4 Bézout Coefficients

Given positive integers a and b, find integers x and y such that `ax + by = gcd(a, b)`.
...
function extended_gcd(a, b):
    if b == 0:
        return (a, 1, 0)  # gcd, x, y where a×1 + 0×0 = a

    gcd, x1, y1 = extended_gcd(b, a mod b)

    # Back-substitute: b×x1 + (a mod b)×y1 = gcd
    # Since a mod b = a - (a ÷ b)×b:
    # Rearranges to: a×y1 + b×(x1 - (a ÷ b)×y1) = gcd
...
**Application**: To find modular inverse of a mod m, solve `ax + my = 1`. The x value (adjusted to be positive) is the inverse.
